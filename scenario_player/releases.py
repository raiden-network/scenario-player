...
def is_executable(path: pathlib.Path) -> bool:
    """Check for a set executable bit on the given pathlib.Path object."""
...
class ReleaseArchive:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        """Close the the archive file object when leaving the `with` context."""
        self.close()
    ...
    def unpack(self, target_dir: pathlib.Path) -> pathlib.Path:
        ...
        We also set the rwx bits on the extracted binary.
        """
