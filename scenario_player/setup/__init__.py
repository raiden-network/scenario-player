...
since we cannot create ether out of thin air (yet).
...
By default, the SP deploys a custom token contract to the specified network, which contains
a function to mint tokens on demand. This means each run starts with a fresh token economy.
...
Raiden Client nodes with its tokens. This is done in a similar fashion to eth
...
UserDepositContract Token Approval Update
------------------------------------------
Depending on the scenario yaml's configuration, we update the approved UDC amount
for our Test Account address.

Typically, each node would request the UDC tokens themselves, but this would require
setting approval amounts for each node, resulting in multiple transaction calls
(which is something we would like to avoid, for the sake of reducing set up time).

Instead, we set the approval amount for the Test Account, and deposit the tokens
from there, into each of the nodes.

..Note::

    The set `approval` **must be less than or equal** to the minted amount of
...
----------------------------------
Once the approval amount has been updated, we can deposit UDC Tokens for each
of the nodes. This is done on a per-node basis. As stated before, we deposit more
