...
def return_available_releases() -> Dict[str, RaidenReleaseInfo]:
    ...
    """
    with REDMetricsTracker(request.method, '/releases'):
        filter = request.args.get('only', 'all')
        if filter and filter not in ('downloaded', 'installed', 'all'):
            flask.abort(404, description="'only' parameter must be one of 'downloaded', 'installed' or 'all'")
...
def latest_release() -> RaidenReleaseInfo:
    ...
    """
    with REDMetricsTracker(request.method, '/release/latest'):
        flask.abort(501)
