...
def TEST_CONFIG_DICT():
    return {"TESTING": True, "ICE_CREAM_FLAVOR": "VANILLA"}
...
class TestConstructFlaskApp:
    @pytest.mark.parametrize(
        "test_config", [None, {"TESTING": True}], ids=["With Test Config", "Without Test Config"]
    )
    @pytest.mark.parametrize(
        "values", [{"SECRET_KEY": "dev", "DATABASE": "default"}, {}], ids=["defaults", "overrides"]
    )
