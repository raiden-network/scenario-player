...
class TestManagedFile_properties:
    def test_exists_locally_property_detects_presence_of_managed_file_on_disk_correctly(
        self, tmp_src_fpath
    ):
    ...
    @mock.patch("scenario_player.utils.files.base.ManagedFile.update_file_references")
    def test_has_symlinks_property_correctly_assert_copy_presence_from_symlinks_attribute(
        self, mock_update_refs, tmp_src_fpath
    ):
...
class TestManagedFile_update_file_references_method:
    def test_yield_unchanged_symlinks_excludes_deleted_symlinks(
        self, tmp_src_fpath, tmp_target_dir
    ):
        """If a symlink was deleted on disk, it should no be yielded by
        :meth:`ManagedFile.yield_unchanged_symlinks`."""
        instance = ManagedFile(tmp_src_fpath)
    ...
    def test_yield_unchanged_symlinks_excludes_modified_symlinks(
        self, tmp_src_fpath, tmp_target_dir
    ):
        ...
        # Unlink the symlink, create a new one with an identical name but point
    ...
    def test_yield_unchanged_copies_excludes_deleted_copies(self, tmp_src_fpath, tmp_target_dir):
        """If a hard-copy was deleted on disk, it should no be yielded by
        :meth:`ManagedFile.yield_unchanged_copies`."""
        instance = ManagedFile(tmp_src_fpath)
    ...
    def test_yield_unchanged_copies_excludes_copies_converted_to_symlinks(
        self, tmp_src_fpath, tmp_target_dir
    ):
    ...
    def test_reference_update_generator_sub_routines_warn_when_dropping_references(
        ...
        """When we drop a reference, a warning is raised indicating the path
        we're dropping and from what attribute."""
        instance = ManagedFile(tmp_src_fpath)
        getattr(instance, attr).add(tmp_target_dir)

        expected_msg = (
            f"Reference {tmp_target_dir.joinpath(tmp_src_fpath.name)} changed on disk"
            f" - dropping it from '{instance}.{attr}'."
        )
        with pytest.warns(ReferenceDropped, matches=expected_msg):
            set(generator_method(instance))

    @pytest.mark.parametrize(
        "generator_method", argvalues=["yield_unchanged_symlinks", "yield_unchanged_copies"]
    )
    def test_update_file_references_calls_yield_unchanged_generator(
        self, generator_method, tmp_src_fpath
    ):
        """The method :meth:`ManagedFile.update_file_references` makes use of
        our generator sub-routines to detect changes."""
        with mock.patch(
            f"scenario_player.utils.files.base.ManagedFile.{generator_method}",
            return_value=iter([]),
        ) as mock_generator:
            instance = ManagedFile(tmp_src_fpath)
...
class TestManagedFileInterface:
    ...
    @pytest.mark.parametrize(
        argnames="method",
        argvalues=[ManagedFile.copy_to_dir, ManagedFile.create_symlink],
    ...
    @pytest.mark.parametrize(
        "create_method", argvalues=[ManagedFile.create_symlink, ManagedFile.copy_to_dir]
    )
    @mock.patch("scenario_player.utils.files.base.ManagedFile.update_file_references")
    ...
    def test_remove_dir_returns_false_and_does_nothing_if_tar_dir_not_in_symlinks_or_copies(
        self, tmp_src_fpath, tmp_target_dir
    ):
