...
def generic_task_test(
    ...
    if expected_req_method:
        assert mocked_responses.calls[-1].request.method == expected_req_method
        assert mocked_responses.calls[-1].request.url == expected_req_url
        assert mocked_responses.calls[-1].request.body.decode() == json.dumps(expected_req_body)

        assert mocked_responses.calls[-1].response.json() == resp_json


def assert_task_test(
    mocked_responses,
    api_task_by_name,
    task_name,
    task_params,
    ...
    # Useful if an exception will be raised before the request is performed
    if expected_req_method:
        if isinstance(resp_code, list) and isinstance(resp_json, list):
            assert len(resp_code) == len(resp_json)
            for a, b in zip(resp_json, resp_code):
                mocked_responses.add(
                    expected_req_method, expected_req_url, json=a, status=b,
                )
        else:
            mocked_responses.add(
                expected_req_method, expected_req_url, json=resp_json, status=resp_code,
    ...
    if expected_req_method:
        ...
        if isinstance(resp_json, list):
            assert mocked_responses.calls[-1].response.json() == resp_json[-1]
        else:
            assert mocked_responses.calls[-1].response.json() == resp_json
