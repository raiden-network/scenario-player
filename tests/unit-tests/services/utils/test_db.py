...
from scenario_player.services.utils.db import JSONRedis, get_db, close_db
...
class TestJSONRedis:
    ...
    def test_tset_method_calls_set_json_with_table_attr_and_given_encode_kwargs(self, mock_set_json, _, __):
    ...
    def test_tget_method_calls_get_json_with_table_attr_and_given_decode_kwargs(self, mock_get_json, _, __):
        instance = JSONRedis('test_table')
    ...
    def test_set_json_calls_hmset_method_with_json_encoded_string(self, patched_hmset, __):
        instance = JSONRedis('test_table')
    ...
    def test_get_json_applies_decoding_options_to_json_string(self, mock_loads, _, patched_hmget):
    ...
    def test_set_json_applies_encoding_options_to_json_string(self, mock_dumps, _, __):
    ...
    @mock.patch(f'{db_import_path}.json.dumps')
    def test_encode_kwargs_passed_directly_to_set_json_take_precedence_over_encoding_options_stored_in_instance(self, mock_dumps, _, __):
        instance = JSONRedis('test_table', encoding_options={'option': 'value'})
        instance.set_json('test_table', 'key', {'sth': 'blah'}, option='nugget')
        mock_dumps.assert_called_once_with({'sth': 'blah'}, option='nugget')

    @mock.patch(f'{db_import_path}.json.loads')
    def test_decode_kwargs_passed_directly_to_get_json_take_precedence_over_decoding_options_stored_in_instance(self, mock_loads, _, patched_hmget):
        patched_hmget.return_value = '{"sth": "blah"}'
        instance = JSONRedis('test_table', decoding_options={'option': 'value'})
    ...
    def test_tset_method_propagates_ValueError_during_json_encoding_from_downstream(self, _, __, ___):
        instance = JSONRedis('test_table')
    ...
    def test_tget_method_raises_CorruptedDBEntry_exception_if_JSONDecodeError_is_raised_downstream(self, _, __, ___):
        instance = JSONRedis('test_table')
        with pytest.raises(CorruptedDBEntry):
            instance.tget('key')

    @mock.patch(f'{db_import_path}.json.dumps', side_effect=ValueError('from mock'))
    def test_set_json_propagates_ValueError_during_json_encoding(self, _, __, ___):
        instance = JSONRedis('test_table')
        with pytest.raises(ValueError, match=r".*from mock.*"):
            instance.set_json('test_table', 'key', {"k": "v"})

    @mock.patch(f'{db_import_path}.json.loads', side_effect=json.JSONDecodeError('from mock', "", 0))
    def test_get_json_propagates_JSONDecodeError(self, _, __, ___):
        instance = JSONRedis('test_table')
