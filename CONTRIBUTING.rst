...
steps need to be taken.
    ...
    2. Install the git hook to run linters in a post-commit hook (`make install-post-commit-hook`).
    3. Create a new branch from **dev** (`git checkout dev && git checkout -b ${NEW_BRANCH_NAME}`).
    4. Implement the feature or fix - be atomic in your commits and do not squash them!
    5. Make sure your fix passes linter checks and the test harness (`make lint && tox`)
    6. Push your commits (`git push -u origin ${NEW_BRANCH_NAME}`
    7. Open a PR, requesting to merge into **`dev`** .
    8. Wait for feedback or approval.

        8.1. **Maintainers only**: After approving a PR, merge it using the following REGEX pattern for the commit title:
...
The following listing outlines the workflow associated with hotfixes:
    ...
    1. Fork the project, if you have not already done so.
    2. Install the git hook to run linters in a post-commit hook (`make install-post-commit-hook`).
    3. Create a new branch from **master** (`git checkout dev && git checkout -b ${NEW_BRANCH_NAME}`).
    4. Implement the feature or fix - be atomic in your commits and do not squash them!
    5. Make sure your fix passes linter checks and the test harness (`make lint && tox`)
    6. Push your commits (`git push -u origin ${NEW_BRANCH_NAME}`
    7. Open a PR, requesting to merge into **`master`** .
    8. Wait for feedback or approval.
    ...
    10. Two new PRs will be opened, containing the same commits, but requesting to merge into `dev` and `release` (if present).
    11. Resolve any conflicts that may arise.
    12. Request a review
...
Once polishing is complete and we're ready to merge/release:
    ...
    4. A PR will be opened, requesting to merge `release` into `dev`
